...
try{
            ...
            $mabase = new PDO('mysql:host=mysql-ccauet.alwaysdata.net;dbname=ccauet_covid', $user, $pass);
...
function afficheFormulaireConnexion($mabase){
    ...
    if( isset($_POST["login"]) && isset($_POST["password"])){
        ...
        $Result = $mabase->query("SELECT * FROM `utilisateur` WHERE `login`='".$_POST['login']."' AND `mdp` = '".$_POST['password']."'");
        if($tab = $Result->fetch()){ 
             //si mdp = ok
            $access = true;
